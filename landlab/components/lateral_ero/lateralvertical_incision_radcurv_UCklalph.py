...
#class LateralIncision(object):
    ...
    def run_one_storm(self, grid, z, vol_lat, rainrate=None, storm_dur=None, qsinlet=None, inlet_area=None, Klr=None, alph=None, runoff=None):
        ...
        if Klr==None:    #Added10/9 to allow changing rainrate (indirectly this way.)
            Klr=self.Klr
        if alph==None:    #Added10/18 to allow changing rainrate (indirectly this way.)
            alph=self.alph
        ...
        kw=10.
        F=0.02 
        #October 18, final change pre defense. passing in runoff, calculating 
        #alpha and kl/kv. need to provide vs
        #runoff=(Klr*F/kw)**2        
        vs=2e-5
        Klr=runoff**0.5*kw/F
        alph=vs/runoff
