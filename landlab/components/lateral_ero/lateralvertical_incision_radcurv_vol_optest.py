...
#class LateralIncision(object):
    ...
    def run_one_storm(self, grid, z, vol_lat, rainrate=None, storm_dur=None, qsinlet=None):
	...
	#vol_diff=np.zeros(grid.number_of_nodes)
        ...
        flowdirs, drain_area, q, max_slopes, s, receiver_link = flow_router.route_flow(elevs=z, node_cell_area=node_area)
        ...
        while time < storm_dur:
            ...
            for i in dwnst_nodes:
                ...
                if i in flowdirs:
                    ...
                    if lat_node!=0.0:
                        ...
                        if z[lat_node] > z[i]:                           
                            ...
                            vol_lat_dt[lat_node]+=abs(petlat)*dx*z_bank                           
                ...
                qsin[flowdirs[i]]+=qsin[i]-(dzver[i]*dx**2)-(petlat*dx*z_bank)   #qsin to next node
                #qsqt[i]=qsin[i]/qt[i]
                     

                if (1 and drain_area[i] > 20000):
                    #if drain_area[i] > 500:
                    print 'node id', i
