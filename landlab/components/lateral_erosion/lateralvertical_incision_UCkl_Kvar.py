...
#        self.Kv = inputs.get('KV_COEFFICIENT', ptype=float)
    ...
    def run_one_storm(self, grid, z, vol_lat, Kvts, rainrate=None, storm_dur=None, qsinlet=None, inlet_area=None, Klr=None):
...
#        Kv=self.Kv
        ...
        Kl=Kv*Klr
#        print 'Kv[100:107]', Kv[100:107]
#        print 'Kv[2500:2510]', Kv[2500:2507]
#        print delta
        dx=grid.dx
	...
	#vol_diff=np.zeros(grid.number_of_nodes)
        ...
        while time < storm_dur:
            ...
            for i in dwnst_nodes:
                ...
                qt[i]=Kv[i]*drain_area[i]**(3./2.)*max_slopes[i]/alph
